...
<?= $content ?>
		...
		<script>
		if(window.Flatpickr) {
			window.Flatpickr.setDefaults({ dateFormat: 'Y-m-d H:i:S', altInput: true, altFormat: 'm/d/Y h:i K' });
		}
		</script>
		<script src="<?= airoute( 'aimeos_shop_jqadm_file', array( 'site' => $site, 'locale' => 'en', 'name' => 'index-js' ) ) ?>"></script>
		<script>
		new MutationObserver(function() {
			document.querySelectorAll('input[placeholder="YYYY-MM-DD hh:mm:ss (optional)"]').forEach(function(el) {
				el.placeholder = 'MM/DD/YYYY hh:mm:ss (optional)';
			});
